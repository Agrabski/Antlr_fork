...
{
	auto const size = (int)decisionToDFA.size();
	decisionToDFA.clear();
	for (int d = 0; d < size; ++d)
		decisionToDFA.push_back(dfa::DFA(atn.getDecisionState(d), d));
}

size_t ParserATNSimulator::adaptivePredict(not_null<TokenStream*> input, size_t decision, ParserRuleContext* outerContext)
{
	...
	auto const m = input->mark();
	auto const index = _startIndex;
	...
	return execATN(dfa, s0, input, index, outerContext != nullptr ? outerContext : &ParserRuleContext::EMPTY);
}

size_t ParserATNSimulator::execATN(dfa::DFA& dfa, dfa::DFAState* s0, not_null<TokenStream*> input, size_t startIndex,
...
std::unique_ptr<ATNConfigSet> ParserATNSimulator::applyPrecedenceFilter(not_null<ATNConfigSet*> configs)
{
	std::map<size_t, Ref<PredictionContext>> statesFromAlt1;
	auto configSet = std::make_unique<ATNConfigSet>(configs->fullCtx);
	assert(configSet != nullptr);
	for (Ref<ATNConfig> const& config : configs->configs)
	{
		// handle alt 1 first
		if (config->alt != 1)
			continue;
...
atn::ATNState* ParserATNSimulator::getReachableTarget(not_null<Transition*> trans, size_t ttype)
{
...
bool ParserATNSimulator::canDropLoopEntryEdgeInLeftRecursiveRule(ATNConfig& config) const
{
	...
	auto const blockEndStateNum = decisionStartState->endState->stateNumber;
...
std::string_view ParserATNSimulator::getRuleName(size_t index)
{
	if (parser != nullptr)
	{
...
Ref<ATNConfig> ParserATNSimulator::actionTransition(ATNConfig& config, ActionTransition* t)
{
#if DEBUG_DFA == 1
...
Ref<ATNConfig> ParserATNSimulator::predTransition(ATNConfig& config, PredicateTransition* pt,
	bool collectPredicates, bool inContext, bool fullCtx)
{
#if DEBUG_DFA == 1
...
Ref<ATNConfig> ParserATNSimulator::ruleTransition(ATNConfig& config, RuleTransition* t)
{
#if DEBUG_DFA == 1
...
std::string ParserATNSimulator::getTokenName(size_t t)
{
	...
	auto displayName = vocabulary.getDisplayName(t);
	if (displayName == std::to_string(t))
	{
		return std::string(displayName);
	}

	return std::string(displayName) + "<" + std::to_string(t) + ">";
}
	...
	for (auto c : nvae.getDeadEndConfigs()->configs)
	{
		...
		if (c->state->transitions.size() > 0)
		{
			...
			else if (is<SetTransition*>(t))
			{
				...
				auto const is_not = is<NotSetTransition*>(st);
				trans = (is_not ? "~" : "");
				trans += "Set ";
