...
void DefaultErrorStrategy::reportError(Parser* recognizer, const RecognitionException& e, ParserRuleContext* currentContext) {
	...
	if (is<const NoViableAltException*>(&e)) {
		...
		reportFailedPredicate(recognizer, static_cast<const FailedPredicateException&>(e), currentContext);
	}
...
void DefaultErrorStrategy::recover(Parser* recognizer, std::exception_ptr /*e*/, ParserRuleContext* currentContext)
{
	...
	misc::IntervalSet followSet = getErrorRecoverySet(recognizer, currentContext);
	consumeUntil(recognizer, followSet, currentContext);
}

void DefaultErrorStrategy::sync(Parser* recognizer, ParserRuleContext* currentContext)
{
	...
	case atn::ATNState::STAR_LOOP_ENTRY:
		...
		if (singleTokenDeletion(recognizer, currentContext) != nullptr)
			return;
		throw InputMismatchException(recognizer, currentContext);

	case atn::ATNState::PLUS_LOOP_BACK:
	case atn::ATNState::STAR_LOOP_BACK:
	{
		reportUnwantedToken(recognizer, currentContext);
		misc::IntervalSet expecting = recognizer->getExpectedTokens(currentContext);
		misc::IntervalSet whatFollowsLoopIterationOrRule = expecting.Or(getErrorRecoverySet(recognizer, currentContext));
		consumeUntil(recognizer, whatFollowsLoopIterationOrRule, currentContext);
	}
	break;
...
void DefaultErrorStrategy::reportUnwantedToken(Parser* recognizer, ParserRuleContext* currentContext) {
	if (inErrorRecoveryMode(recognizer)) {
		return;
	}
...
void DefaultErrorStrategy::reportMissingToken(Parser* recognizer, ParserRuleContext* currentContext)
{
	if (inErrorRecoveryMode(recognizer)) {
		return;
	}
...
Token* DefaultErrorStrategy::recoverInline(Parser* recognizer, ParserRuleContext* currentContext) {
	...
	Token* matchedSymbol = singleTokenDeletion(recognizer, currentContext);
	if (matchedSymbol) {
	...
	if (singleTokenInsertion(recognizer, currentContext)) {
		return getMissingSymbol(recognizer, currentContext);
	}
...
bool DefaultErrorStrategy::singleTokenInsertion(Parser* recognizer, ParserRuleContext* currentContext) {
	...
	misc::IntervalSet expectingAtLL2 = atn.nextTokens(next, currentContext);
	if (expectingAtLL2.contains(currentSymbolType)) {
		reportMissingToken(recognizer, currentContext);
		return true;
	}
	return false;
}

Token* DefaultErrorStrategy::singleTokenDeletion(Parser* recognizer, ParserRuleContext* currentContext)
{
	size_t nextTokenType = recognizer->getInputStream()->LA(2);
	misc::IntervalSet expecting = getExpectedTokens(recognizer, currentContext);
	if (expecting.contains(nextTokenType)) {
		reportUnwantedToken(recognizer, currentContext);
		recognizer->consume(currentContext); // simply delete extra token
...
Token* DefaultErrorStrategy::getMissingSymbol(Parser* recognizer, ParserRuleContext* currentContext) {
...
misc::IntervalSet DefaultErrorStrategy::getExpectedTokens(Parser* recognizer, ParserRuleContext* currentContext)
{
	return recognizer->getExpectedTokens(currentContext);
}
...
misc::IntervalSet DefaultErrorStrategy::getErrorRecoverySet(Parser* recognizer, ParserRuleContext* currentContext) {
	const atn::ATN& atn = recognizer->getInterpreter<atn::ATNSimulator>()->atn;
	RuleContext* ctx = currentContext;
...
void DefaultErrorStrategy::consumeUntil(Parser* recognizer, const misc::IntervalSet& set, ParserRuleContext* currentContext) {
